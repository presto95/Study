...
레이아웃 객체는 컬렉션 뷰의 레이아웃을 만들기 위해 데이터 소스가 제공한 정보를 사용한다. 레이아웃은 `collectionView` 프로퍼티에 대한 메소드를 호출하여 데이터 소스와 소통한다. 모든 레이아웃의 메소드 내에서 이 프로퍼티에 접근 가능하다. 컬렉션 뷰가 레이아웃 프로세스 동안 