...
커스텀 컬렉션 뷰 레이아웃을 만드는 것은 직관적인 요구사항이 있어 간단하다. 하지만 프로세스의 세부적인 구현은 다양할 수 있다. 레이아웃은 컬렉션 뷰가 포함하는 모든 뷰에 대한 레이아웃 애트리뷰트 객체를 반드시 만들어야 한다. 이 특성들이 만들어지는 순서는 앱의 특성에 달려 있다. 수천 개의 아이템을 나타내는 컬렉션 뷰의 경우 사전에 레이아웃 특성을 계산하고 캐싱하는 것은 시간을 낭비하는 프로세스다. 그러므로 오직 특정 아이템에 대해서 요청할 때만 특성을 만드는 것이 더 낫다. 더 적은 아이템을 가진다면 일단 레이아웃 정보를 계산하고 캐싱하여 특성을 요청할 때마다 참조하는 것이 많은 불필요한 재계산을 하지 않게 해줄 수 있다.이 챕터에서 예제는 두 번째 카테고리에 관한 것이다.
...
이제 필요한 모든 컴포넌트들이 초기화되었고, 레이아웃을 준비할 수 있다. 컬렉션 뷰는 먼저 레이아웃 프로세스 동안 `prepare()` 메소드를 호출한다. 예제에서 `prepare()` 메소드는 컬렉션 뷰의 모든 뷰에 대한 모든 레이아웃 애트리뷰트 객체의 인스턴스를 만들기 위해 사용되며, 나중에 사용하기 위해 `layoutInformation`  딕셔너리에 그 특성들을 캐시한다. Preparing the Layout에서 `prepare()` 메소드에 대한 더 많은 정보를 확인하라.
